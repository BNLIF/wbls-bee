...
void MCReader(char* ratfile, int eventNo, TString option){
    int tmpcode;

    cout << "start: " << flush; tmpcode = system("date");

    DSReader r(ratfile);
    DS::Root *ds = r.GetEvent(eventNo);
    ...
    TString eventDir = TString::Format("data/%i", eventNo);
    gSystem->mkdir(eventDir.Data(), kTRUE);

    TString jsonFileName = TString::Format("%s/%i-%s.json", eventDir.Data(), eventNo, option.Data());
